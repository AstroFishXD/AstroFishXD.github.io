...
/* =========================================================
   ...
   muestran en pantalla (edítalos a mano cuando quieras).
========================================================= */
const CONFIG = {
  youtube: {
    channelId: "PON_AQUI_TU_CHANNEL_ID", // Ej: UCxxxxxxxxxxxxxxxxxxxxxx (NO es el @handle)
...
/* =========================================================
   ...
   - videoUrl:      link al video de YouTube
   ...
   - downloads:     lista de botones de descarga, cada uno con
                     ...
                     Puedes dejar el arreglo vacío [] si el video
    ...
    <article class="card" data-tag="${item.tag}">
      ...
      <div class="card__body">
        ...
        <div class="card__actions">
          <a class="btn btn--primary btn--small" href="${item.videoUrl}" target="_blank" rel="noopener">Ver video</a>
          ${downloadsHTML}
        </div>
      </div>
    </article>
...
/** Pinta la sección "Videos": todo item con section: "video", sin filtros */
function renderVideos() {
  const grid = document.getElementById("videos-grid");
  const items = contentData.filter((item) => item.section === "video");

  if (items.length === 0) {
    grid.innerHTML = `<p class="cards-grid__empty">Todavía no hay videos cargados.</p>`;
    return;
  }
...
/** Conecta los botones de filtro: click -> re-renderiza sin recargar la página */
...
/* =========================================================
   ...
   VÍA A — YouTube Data API v3 (oficial, gratis, con cuota diaria)
     ...
     Cloudflare Workers, etc.) que guarde la key en el server
...
async function fetchYouTubeStats() {
  ...
  try {
    ...
    */
    ...
    // Mientras el bloque de arriba está comentado, cae aquí:
    el.textContent = CONFIG.youtube.fallbackCount;
    elBig.textContent = CONFIG.youtube.fallbackCount;
  } catch (err) {
...
async function fetchTikTokStats() {
  ...
  try {
    ...
    // el que te de tu servicio de RapidAPI, etc.)
    /*
    const res = await fetch(`https://TU-ENDPOINT.example.com/tiktok/${CONFIG.tiktok.username}`);
    const data = await res.json();
